...
namespace Stribog_Crypto
{
    ...
    class StriBOG
    {
        ...
        private byte[] tmp = new byte[64], 
            N = new byte[64], // начальный вектор
            Sigm = new byte[64]; 
        public int dlina = 0;
        ...
        /// <summary>
        /// Дополнение нулями бит массивы байт(бит).
        /// </summary>
        private byte[] SlozheniePo512Module(byte[] a, byte[] b) 
        {
        ...
        private byte[] HOR(byte[] a, byte[] b)
        {
        ...
        /// <summary>
        ///  Каждый байт из 512-битной входной последовательности заменяется соответствующим байтом из таблицы подстановок Pi
        /// </summary>
        /// <param name="state">Байты последовательности для замены.</param>
        /// <returns></returns>
        private byte[] S(byte[] state) // 0x02 00000010
        {
        ...
        /// <summary>
        /// Для каждой пары байт из входной последовательности происходит замена одного байта другим.
        /// </summary>
        /// <param name="state">Байты последовательности для замены.</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Умножение 64-битного входного вектора на бинарную матрицу A размерами
        /// </summary>
        ...
        private byte[] L(byte[] state) // 
        {
            ...
            {
                ulong t = 0;
                byte[] for_tmp_bytes = new byte[8];
                Array.Copy(state, i * 8, for_tmp_bytes, 0, 8);
                for_tmp_bytes = for_tmp_bytes.Reverse().ToArray();

                BitArray bytes_for_matrix = new BitArray(for_tmp_bytes);

                bool[] current_bytes = new bool[64];
                bytes_for_matrix.CopyTo(current_bytes, 0);

                current_bytes = current_bytes.Reverse().ToArray();
                for (int j = 0; j < 64; j++)
                {
                    if (current_bytes[j] != false)
                        t = t ^ A[j];
                }
        ...
        /// <summary>
        /// Генерация байт на каждом i проходе.
        /// </summary>
        /// <param name="K">Входные байты</param>
        /// <param name="i">Текущий проход</param>
        /// <returns>Массив байт после каждой итерации.</returns>
        private byte[] GenerateNewTempKey(byte[] K, int i) 
        {
            K = HOR(K, ConstTable[i]);
            K = S(K);
            K = P(K);
            K = L(K);
            return K;
        ...
        /// <summary>
        /// Функция сжатия согласно ГОСТу 34-11.2012
        /// </summary>
        /// <param name="N"></param>
        /// <param name="h"></param>
        /// <param name="m"></param>
        /// <returns></returns>
        private byte[] Func_Szhatiya(byte[] N, byte[] h, byte[] m)
        {
            byte[] K = HOR(h, N);
            K = S(K);
            K = P(K);
        ...
        public byte[] HashProgram(byte[] message)
        {
            int len = message.Length * 8; // длина в байтах
            byte[] lastblock = new byte[64]; // выделение памяти для последнего блока - чтобы был 01 (против атаки)
            byte[] h = new byte[64]; 
            Array.Copy(tmp, h, 64);
            if (dlina == 512)
            {
            ...
            byte[] N_512 = BitConverter.GetBytes(512); // первый для всех будущих блоков
            int lv_i = 0;
            while (len >= 512) // постепенно обрезаем, т.е. сжимаем, если больше
            {
                lv_i++;
                int dlina = lv_i * 64; // вычитаемая длина
                byte[] new_massiv = new byte[64];
                Array.Copy(message, message.Length - dlina, new_massiv, 0, 64);
                h = Func_Szhatiya(N, h, new_massiv); // применяем ф-ию сжатия
                N = SlozheniePo512Module(N, N_512.Reverse().ToArray()); // обязательный реверс
                Sigm = SlozheniePo512Module(Sigm, new_massiv);
                len -= 512;
            }

            byte[] do_reversa_v_konec = new byte[message.Length - lv_i * 64];
            Array.Copy(message, 0, do_reversa_v_konec, 0, message.Length - lv_i * 64);
            if (do_reversa_v_konec.Length < 64) // доводим до 64 байт, чтобы был блок
            {
                for (int i = 0; i < (64 - do_reversa_v_konec.Length - 1); i++) // заполнение нулями
                {
                    lastblock[i] = 0;
                }
                lastblock[64 - do_reversa_v_konec.Length - 1] = 0x01; // чтобы избежать атаки(когда тупо заполняем нулями)
                Array.Copy(do_reversa_v_konec, 0, lastblock, 64 - do_reversa_v_konec.Length, do_reversa_v_konec.Length);
            }
            h = Func_Szhatiya(N, h, lastblock);
            byte[] MesLen = BitConverter.GetBytes(do_reversa_v_konec.Length * 8);
            N = SlozheniePo512Module(N, MesLen.Reverse().ToArray());
            Sigm = SlozheniePo512Module(Sigm, lastblock);
            // для последних двух итераций
            byte[] Nulevoj ={
            ...
            h = Func_Szhatiya(Nulevoj, h, N);
            h = Func_Szhatiya(Nulevoj, h, Sigm);

            if (dlina == 512) return h;
            else
            {
